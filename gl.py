...
def reflectVector(normal, dirVector):
    # R = 2 * (N dot L) * N - L
    reflect = 2 * op.dot(normal, dirVector)
    reflect = op.mtxmul(reflect, normal)
    reflect = op.subtract(reflect, dirVector)
    reflect = op.divide(reflect , op.norm(reflect))
    return reflect
...
def refractVector(N, I, ior):
    ...
    R = op.add(op.mtxmul(eta , I) , op.mtxmul((eta * cosi - k**0.5),  N))
    return R / op.norm(R)
...
class Raytracer(object):
    ...
    def castRay(self, orig, direction, origObj = None, recursion = 0):
        ...
        # Direccion de vista
        view_dir = op.subtract(self.camPosition, intersect.point)
        view_dir = op.divide(view_dir , op.norm(view_dir))
        ...
        if self.dirLight:
            ...
            light_dir = op.mtxmul(-1,self.dirLight.direction)

            # Calculamos el valor del diffuse color
            intensity = self.dirLight.intensity * max(0, op.dot(light_dir, intersect.normal))
            ...
            spec_intensity = self.dirLight.intensity * (max(0, op.dot(view_dir, reflect)) ** material.spec)
            ...
            dirLightColor = op.mtxmul(1 - shadow_intensity , op.add(diffuseColor , specColor))
        ...
        for pointLight in self.pointLights:
            ...
            # spec_intensity: lightIntensity * ( view_dir dot reflect) ** especularidad
            spec_intensity = pointLight.intensity * (max(0, op.dot(view_dir, reflect)) ** material.spec)
            ...
            shadMat, shadInter = self.scene_intercept(intersect.point,  light_dir, intersect.sceneObject)
            if shadInter is not None and shadInter.distance < op.norm(op.subtract(pointLight.position, intersect.point)):
                shadow_intensity = 1

            pLightColor = op.add( pLightColor, op.mtxmul(1 - shadow_intensity, op.add(diffuseColor , specColor)))

        
        if material.matType == OPAQUE:
            # Formula de iluminacion, PHONG
            finalColor = op.add(ambientColor ,op.add( dirLightColor , pLightColor))
            ...
            if material.texture and intersect.texCoords:
                ...
                finalColor = op.mtxmul(finalColor, [
                                        texColor[2] / 255,
                                        texColor[1] / 255,
                                        texColor[0] / 255
                                        ])
            ...
            reflect = reflectVector(intersect.normal, op.mtxmul(-1,direction))
            reflectOrig = op.add(intersect.point, bias) if outside else op.subtract(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrig = op.subtract(intersect.point, bias) if outside else op.add(intersect.point, bias)
            ...
            finalColor = op.add(op.mtxmul(kr,reflectColor), op.mtxmul(1-kr, refractColor))



        # Le aplicamos el color del objeto
        finalColor =op.mtxmul(finalColor, objectColor)
